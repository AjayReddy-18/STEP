...
function areBothMatricesEqual(matrix1, matrix2, left, right) {
  if (right < left) {
    return true;
  }

  const areLeftArraysEqual = areEqual(matrix1[left], matrix2[left]);
  const areRightArraysEqual = areEqual(matrix1[right], matrix2[right]);

  if (!areLeftArraysEqual || !areRightArraysEqual) {
    return false;
  }

  return areBothMatricesEqual(matrix1, matrix2, left + 1, right - 1);
}

function areMatricesEqual(matrixA, matrixB) {
  if (Object.is(matrixA, matrixB)) {
    return true;
  }
  ...
  return areBothMatricesEqual(matrixA, matrixB, 0, matrixA.length - 1);
}

function testFunction(input, expected, functionToTest) {
  const actual = functionToTest(...input);
  const mark = areMatricesEqual(expected, actual) ? '✅' : '❌';
