...
const doubleLettersOf = function (strings) {
  return strings.map(function (string) {
    const stringInArray = string.split('');
    return stringInArray.reduce(function (string, char) {
      return string + char.repeat(2);
    }, '');
  });
};
...
const countVowelsOf = function (strings) { 
  return strings.map(function (string) {
    const stringInArray = string.split('');
    return stringInArray.reduce(function (vowelCount, char) {
      return 'aeiou'.includes(char.toLowerCase()) ? vowelCount + 1 : vowelCount;
    }, 0)
  })
};
...
const withoutVowelsOf = function (strings) { 
  return strings.map(function (string) {
    return ['a', 'e', 'i', 'o', 'u'].reduce(function (string, vowel) {
      return string.replaceAll(vowel, '');
    }, string);
  })
};
...
const cumulativeSumsOf = function (arrays) { 
  return arrays.map(function (array) {
    return array.reduce(function (array, number) {
      const cumulativeSum = array.at(-1) || 0;
      return array.concat(cumulativeSum + number);
    }, [])
  })
};
...
const reversedWordsOf = function (strings) { 
  return strings.map(function (string) {
    ...
    return words.map(function (word) {
      ...
      return wordInArray.reverse().join('');
    }).join(' ');
  })
};
...
const sortStudentsByAverage = function (students) { 
  return students.map(function (student) {
    const average = (student.grades.reduce(function (total, grade) {
      return total + grade;
...
console.log(sortStudentsByAverage([{ name: "Alice", grades: [80, 90, 85] }, { name: "Bob", grades: [70, 75, 80] }]));
